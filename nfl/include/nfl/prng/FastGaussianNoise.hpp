...
namespace nfl {
  ...
  template<typename out_class>
  class GaussianNoise {
  public:
    virtual void getNoise(out_class * const rand_data2out, uint64_t rlen) = 0;
  };

  template<class in_class, class out_class, unsigned _lu_depth>
  class FastGaussianNoise : public GaussianNoise<out_class> {
  ...
  template <typename out_class>
  struct PalisadeGaussianGenerator : GaussianNoise <out_class>
  {
    ...
    PalisadeGaussianGenerator (float std) :
    ...
    unsigned int FindInVector(const std::vector<double> &S, double search) const {
    ...
    int32_t GenerateInt ()
    {
      ...
      seed = distribution(generator) - 0.5; //we need to use the binary uniform generator rather than regular continuous distribution; see DG14 for details
	...
	{
	  rand_data2out[i] = static_cast<out_class> (GenerateInt ());
	}
    }
  };
