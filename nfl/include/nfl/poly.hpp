...
namespace nfl {
  ...
  struct gaussian {
    GaussianNoise<out_class> *fg_prng;
    uint64_t amplifier;
    gaussian(GaussianNoise<out_class> *prng) : fg_prng{prng}, amplifier{1} {}
    gaussian(GaussianNoise<out_class> *prng, uint64_t amp) : fg_prng{prng}, amplifier{amp} {}
  };
   ...
   */
  template<class params_t, size_t Degree, size_t NbModuli, size_t IndMod0>
  class poly {

    template <class P> friend class tests::poly_tests_proxy;

    static constexpr size_t N = Degree * NbModuli;
    typename params_t::value_type _data[N] __attribute__((aligned(32)));

  public:
    using main_double = typename params_t::main_double;
    using main_greater_double = typename params_t::main_greater_double;
    using value_type = typename params_t::value_type;
    using greater_value_type = typename params_t::greater_value_type;
    using signed_value_type = typename params_t::signed_value_type;
    ...
    static constexpr size_t indmod0 = IndMod0;
    static constexpr size_t nbits = params_t::kModulusBitsize;
  ...
  public:
    ...
    template <class in_class, unsigned _lu_depth> poly(gaussian<in_class, value_type, _lu_depth> const& mode);
    ...
    template <class in_class, unsigned _lu_depth> void set(gaussian<in_class, value_type, _lu_depth> const& mode);
    ...
    template <class in_class, unsigned _lu_depth> poly& operator=(gaussian<in_class, value_type, _lu_depth> const& mode) { set(mode); return *this; }
    ...
    static constexpr value_type get_modulus(size_t n) { return base.params.P[n+indmod0]; }
    ...
    void serialize_manually(std::ostream& outputstream) {
      outputstream.write(reinterpret_cast<char*>(_data), N * sizeof(value_type));
    }
    void deserialize_manually(std::istream& inputstream) {
      inputstream.read(reinterpret_cast<char*>(_data), N * sizeof(value_type));
    }
    ...
    public:
      ...
      bool ntt(value_type* x, const value_type* wtab, const value_type* winvtab, value_type  const p);
      bool inv_ntt(value_type *x, const value_type* const inv_wtab, const value_type* const inv_winvtab, value_type invK, value_type const p);
	...
	*shoupinvomegas[nmoduli]  __attribute__((aligned(32))),
	invpolyDegree[nmoduli]  __attribute__((aligned(32)));
      ...
      void initialize();
      void prep_wtab(value_type* wtab, value_type* winvtab, value_type w, size_t cm);
  ...
  void sub(poly<T, Degree, NbModuli, IndMod0> & out,
	   poly<T, Degree, NbModuli, IndMod0> const &arg0, poly<T, Degree, NbModuli, IndMod0> const & arg1) {
    out = arg0 - arg1;
  }
   ...
   */
  template<class T, size_t Degree, size_t NbModuli, size_t IndMod0>
  void add(poly<T, Degree, NbModuli, IndMod0> & out,
	   poly<T, Degree, NbModuli, IndMod0> const &arg0, poly<T, Degree, NbModuli, IndMod0> const & arg1) {
    out = arg0 + arg1;
  }
  template<class T, size_t Degree, size_t NbModuli, size_t IndMod0>
  void mul(poly<T, Degree, NbModuli, IndMod0> & out,
	   poly<T, Degree, NbModuli, IndMod0> const &arg0, poly<T, Degree, NbModuli, IndMod0> const & arg1) {
    out = arg0 * arg1;
  }
  ...
  /* misc type adaptor
   */
  // template<class T, size_t Degree, size_t AggregatedModulusBitSize, size_t IndMod0>
  // using poly_from_modulus = poly<T, Degree, AggregatedModulusBitSize / params<T>::kModulusBitsize, IndMod0>;

  /* stream operator
   */
  template<class T, size_t Degree, size_t NbModuli, size_t IndMod0>
  std::ostream& operator<<(std::ostream& os, nfl::poly<T, Degree, NbModuli, IndMod0> const& p);
