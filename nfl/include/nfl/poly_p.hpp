...
class poly_p {
    typedef poly<params_t, Degree, NbModuli, IndMod0> poly_type;
...
public:
  ...
  static constexpr size_t degree = poly_type::degree;
    static constexpr size_t indmod0 = poly_type::indmod0;
...
public:
 ...
 poly_p& operator=(std::initializer_list<value_type> values) {
   poly_obj() = std::forward<std::initializer_list<value_type>>(values);
   return *this;
 }
  ...
  static constexpr value_type get_modulus(size_t n) { return poly_type::get_modulus(n+indmod0); }
  ...
  */
  ...
  template <class in_class, unsigned _lu_depth> void set(gaussian<in_class, value_type, _lu_depth> const& mode) { poly_obj().set(mode); };
 ...
 */
  ...
  // using poly_p_from_modulus = poly_p<T, Degree, AggregatedModulusBitSize / params<T>::kModulusBitsize, IndMod0>;

/* stream operator
 */
  template<class T, size_t Degree, size_t NbModuli, size_t IndMod0>
  std::ostream& operator<<(std::ostream& os, nfl::poly_p<T, Degree, NbModuli, IndMod0> const& p)
{
...
/* unary operators
*/
  template<class E, class T, size_t Degree, size_t NbModuli, size_t IndMod0>
  auto shoup(E const& e, poly_p<T, Degree, NbModuli, IndMod0> const& m) -> decltype(shoup(e, m.poly_obj()))
{
  return shoup(e, m.poly_obj());
}

  template<class T, size_t Degree, size_t NbModuli, size_t IndMod0>
  auto compute_shoup(poly_p<T, Degree, NbModuli, IndMod0> const& p) -> decltype(compute_shoup(p.poly_obj()))
{
...
#define DECLARE_BINARY_OPERATOR_P(SYM, NAME)\
  template<class T, size_t Degree, size_t NbModuli, size_t IndMod0>			\
  auto SYM(nfl::poly_p<T, Degree, NbModuli, IndMod0> const& op0, nfl::poly_p<T, Degree, NbModuli, IndMod0> const& op1) -> decltype(ops::make_op<ops::NAME<nfl::poly_p<T, Degree, NbModuli, IndMod0>, CC_SIMD>>(op0, op1)) { \
  return ops::make_op<ops::NAME<nfl::poly_p<T, Degree, NbModuli, IndMod0>, CC_SIMD>>(op0, op1);\
}\
  template<class T, size_t Degree, size_t NbModuli, size_t IndMod0, class Op, class...Args> \
  auto SYM(nfl::poly_p<T, Degree, NbModuli, IndMod0> const& op0, nfl::ops::expr<Op, Args...> const& op1) -> decltype(ops::make_op<ops::NAME<nfl::poly_p<T, Degree, NbModuli, IndMod0>, typename nfl::ops::expr<Op, Args...>::simd_mode>>(op0, op1)) { \
  return ops::make_op<ops::NAME<nfl::poly_p<T, Degree, NbModuli, IndMod0>, typename nfl::ops::expr<Op, Args...>::simd_mode>>(op0, op1);\
}\
  template<class T, size_t Degree, size_t NbModuli, size_t IndMod0, class Op, class...Args> \
  auto SYM(nfl::ops::expr<Op, Args...> const& op0, nfl::poly_p<T, Degree, NbModuli, IndMod0> const& op1) -> decltype(ops::make_op<ops::NAME<nfl::poly_p<T, Degree, NbModuli, IndMod0>, typename nfl::ops::expr<Op, Args...>::simd_mode>>(op0, op1)){ \
  return ops::make_op<ops::NAME<nfl::poly_p<T, Degree, NbModuli, IndMod0>, typename nfl::ops::expr<Op, Args...>::simd_mode>>(op0, op1);\
}
