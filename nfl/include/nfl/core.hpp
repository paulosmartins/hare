 ...
 */
  ...
  template<class Op, class... Args> inline poly<T, Degree, NbModuli, IndMod0>::poly(ops::expr<Op, Args...> const& expr) {
  *this = expr;
}

  template<class T, size_t Degree, size_t NbModuli, size_t IndMod0>
  template<class Op, class... Args> inline poly<T, Degree, NbModuli, IndMod0>& poly<T, Degree, NbModuli, IndMod0>::operator=(ops::expr<Op, Args...> const& expr) {
  ...
  template<class T, size_t Degree, size_t NbModuli, size_t IndMod0>
inline poly<T, Degree, NbModuli, IndMod0>::operator bool() const {
  return std::find_if(begin(), end(),
                      [](value_type v) { return v != 0; }) != end();
}

  template<class T, size_t Degree, size_t NbModuli, size_t IndMod0>
  typename poly<T, Degree, NbModuli, IndMod0>::core poly<T, Degree, NbModuli, IndMod0>::base;

// *********************************************************
//    Constructor
// *********************************************************

  template<class T, size_t Degree, size_t NbModuli, size_t IndMod0> poly<T, Degree, NbModuli, IndMod0>::core::core()
{
  assert(degree <= params.kMaxPolyDegree); //core: CRITICAL, degree is not lower or equal than kMaxPolyDegree (see params.hpp)"
  initialize();
}

  template<class T, size_t Degree, size_t NbModuli, size_t IndMod0>
  poly<T, Degree, NbModuli, IndMod0>::poly() : poly(std::integral_constant<value_type, 0>::value) {
}

  template<class T, size_t Degree, size_t NbModuli, size_t IndMod0>
  poly<T, Degree, NbModuli, IndMod0>::poly(value_type v, bool reduce_coeffs) {
  set(v, reduce_coeffs);
}

  template<class T, size_t Degree, size_t NbModuli, size_t IndMod0>
  poly<T, Degree, NbModuli, IndMod0>::poly(std::initializer_list<value_type> values, bool reduce_coeffs) {
  set(values, reduce_coeffs);
}

  template<class T, size_t Degree, size_t NbModuli, size_t IndMod0>
template<class It>
  poly<T, Degree, NbModuli, IndMod0>::poly(It first, It last, bool reduce_coeffs) {
  set(first, last, reduce_coeffs);
}

  template<class T, size_t Degree, size_t NbModuli, size_t IndMod0>
  void poly<T, Degree, NbModuli, IndMod0>::set(value_type v, bool reduce_coeffs) {
  if (v == 0) {
  ...
  template<class T, size_t Degree, size_t NbModuli, size_t IndMod0>
  void poly<T, Degree, NbModuli, IndMod0>::set(std::initializer_list<value_type> values, bool reduce_coeffs) {
  set(values.begin(), values.end(), reduce_coeffs);
}

  template <class T, size_t Degree, size_t NbModuli, size_t IndMod0>
template <class It>
  void poly<T, Degree, NbModuli, IndMod0>::set(It first, It last, bool reduce_coeffs) {
  // CRITICAL: the object must be 32-bytes aligned to avoid vectorization issues
  assert((unsigned long)(this->_data) % 32 == 0);
...
// uniformly random, else the coefficients are uniform below the bound
  ...
  template<class T, size_t Degree, size_t NbModuli, size_t IndMod0>
  poly<T, Degree, NbModuli, IndMod0>::poly(uniform const& u) {
  set(u);
}

  template<class T, size_t Degree, size_t NbModuli, size_t IndMod0>
  void poly<T, Degree, NbModuli, IndMod0>::set(uniform const &) {
  // CRITICAL: the object must be 32-bytes aligned to avoid vectorization issues
  assert((unsigned long)(this->_data) % 32 == 0);
  ...
  template<class T, size_t Degree, size_t NbModuli, size_t IndMod0>
  poly<T, Degree, NbModuli, IndMod0>::poly(non_uniform const& mode) {
  set(mode);
}

  template<class T, size_t Degree, size_t NbModuli, size_t IndMod0>
  void poly<T, Degree, NbModuli, IndMod0>::set(non_uniform const& mode) {
  // CRITICAL: the object must be 32-bytes aligned to avoid vectorization issues
  assert((unsigned long)(this->_data) % 32 == 0);
...
template<class in_class, unsigned _lu_depth>
  poly<T, Degree, NbModuli, IndMod0>::poly(gaussian<in_class, value_type, _lu_depth> const& mode) {
  set(mode);
}

  template<class T, size_t Degree, size_t NbModuli, size_t IndMod0>
template<class in_class, unsigned _lu_depth>
  void poly<T, Degree, NbModuli, IndMod0>::set(gaussian<in_class, value_type, _lu_depth> const& mode) {
  // CRITICAL: the object must be 32-bytes aligned to avoid vectorization issues
  assert((unsigned long)(this->_data) % 32 == 0);
...
// *********************************************************
  ...
  template<class T, size_t Degree, size_t NbModuli, size_t IndMod0>
  std::ostream& operator<<(std::ostream& outs, poly<T, Degree, NbModuli, IndMod0> const& p)
{
  bool first = true;
  using value_type = typename poly<T, Degree, NbModuli, IndMod0>::value_type;
  std::string term;
  if (typeid(value_type) == typeid(uint64_t)) term = "ULL"; 
  else if (typeid(value_type) == typeid(uint32_t)) term = "UL"; 
  else term = "U"; 
...
// The NTT is computed in-place on x, so x is the output
  template<class T, size_t Degree, size_t NbModuli, size_t IndMod0> bool poly<T, Degree, NbModuli, IndMod0>::core::ntt(value_type* x, const value_type* wtab, const value_type* winvtab, value_type const p)
{
#ifdef CHECK_STRICTMOD
  for (size_t i = 0 ; i < degree ; i++)
  {
  ...
  for (size_t r = 0; r < M; r++, x += 4)
  {
    ...
    value_type q = ((greater_value_type) t * winvtab[1]) >> params.kModulusRepresentationBitsize;
...
// inverse of the polynomialDegree
  template<class T, size_t Degree, size_t NbModuli, size_t IndMod0> inline bool poly<T, Degree, NbModuli, IndMod0>::core::inv_ntt(value_type * x, const value_type* const inv_wtab, const value_type* const inv_winvtab,
...
// It is used by initialize() and results are used by ntt and inv_ntt
  template<class T, size_t Degree, size_t NbModuli, size_t IndMod0> inline void poly<T, Degree, NbModuli, IndMod0>::core::prep_wtab(value_type* wtab, value_type* wtabshoup, value_type w, size_t cm)
{
  using poly_type = poly<T, Degree, NbModuli, IndMod0>;
  auto const p = get_modulus(cm);
  unsigned K = degree;
  ...
  while (K >= 2)
  {
    ...
    {
      *wtab++ = wi;
      *wtabshoup++ = ((greater_value_type) wi << params.kModulusRepresentationBitsize) / p;
      wi = ops::mulmod<poly_type, simd::serial>{}(static_cast<value_type>(wi), w, cm + IndMod0);
    }
    w = ops::mulmod<poly_type, simd::serial>{}(w, w, cm + IndMod0);
    K /= 2;
  }
}
// *****************************************************************
...
// by phi powers before doing the NTT
  template<class T, size_t Degree, size_t NbModuli, size_t IndMod0> inline void poly<T, Degree, NbModuli, IndMod0>::core::ntt_pow_phi(poly& op)
{
...
// we must multiply the polynomial coefficients by invphi powers before doing the inverse-NTT
  template<class T, size_t Degree, size_t NbModuli, size_t IndMod0> inline void poly<T, Degree, NbModuli, IndMod0>::core::invntt_pow_invphi(poly &op)
{
  for(size_t currentModulus = 0; currentModulus < nmoduli; ++currentModulus) {
    poly::core::inv_ntt(&op(currentModulus, 0), invomegas[currentModulus], shoupinvomegas[currentModulus], invpolyDegree[currentModulus+indmod0], get_modulus(currentModulus));
  }
...
// time through meta-programming and statically defined.
  template<class T, size_t Degree, size_t NbModuli, size_t IndMod0> void  poly<T, Degree, NbModuli, IndMod0>::core::initialize()
{
  using poly_type = poly<T, Degree, NbModuli, IndMod0>;
  for(size_t currentModulus = 0; currentModulus < nmoduli; ++currentModulus)
  {
    ...
    phi = params.primitive_roots[currentModulus + indmod0];
    for (unsigned int i = 0 ;
	 i < round(log2(params.kMaxPolyDegree)) - static_log2<degree>::value ; i++)
    {
      phi = ops::mulmod<poly_type, simd::serial>{}(phi, phi, currentModulus + indmod0);
    }
    ...
    for (unsigned int i = 0 ; i < degree ; i++)
    {
      ...
      shoupphis[currentModulus][i] = ((greater_value_type) temp << params.kModulusRepresentationBitsize) / get_modulus(currentModulus);
      // phi**(i+1)
      temp = ops::mulmod<poly_type, simd::serial>{}(temp, phi, currentModulus + indmod0);
    }
    // At the end of the loop temp = phi**degree

    // Computation of invphi
    // phi**(2*polydegree)=1 -> temp*phi**(degree-1) = phi**(-1)
    invphi = ops::mulmod<poly_type, simd::serial>{}(temp, phis[currentModulus][degree-1], currentModulus + indmod0);

    // Computation of the inverse of degree using the inverse of kMaxPolyDegree
    invpolyDegree[currentModulus] = ops::mulmod<poly_type, simd::serial>{}(params.invkMaxPolyDegree[currentModulus + indmod0],
        static_cast<value_type>(params.kMaxPolyDegree/degree), currentModulus + indmod0);
    ...
    for (unsigned int i = 0 ; i < degree ; i++)
    {
      ...
      shoupinvpoly_times_invphis[currentModulus][i] = ((greater_value_type) temp << params.kModulusRepresentationBitsize)
        / get_modulus(currentModulus);
      // This is invpolyDegree*invphi**(i+1)
      temp = ops::mulmod<poly_type, simd::serial>{}(temp, invphi, currentModulus + indmod0);
    }

    // For the omegas it is easy, we just use the function of David Harvey modified for our needs
    omega = ops::mulmod<poly_type, simd::serial>{}(phi, phi, currentModulus + indmod0);
    prep_wtab(omegas[currentModulus], shoupomegas[currentModulus], omega, currentModulus);

    // And again for the invomegas
    invomega = ops::mulmod<poly_type, simd::serial>{}(invphi, invphi, currentModulus + indmod0);
