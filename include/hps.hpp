...
struct context_hps_t
{
  ...
  //class that implements BEHZ and HPS, but with different methods
  ...
  context_hps_t()
    : cont_RNS(new context_rns_t(true))
  {
  }

  context_hps_t(const context_hps_t &other)
    : cont_RNS(new context_rns_t())
  {
    *cont_RNS = *other.cont_RNS;
  }
  
  context_hps_t &operator=(const context_hps_t &other)
  {
    *cont_RNS = *other.cont_RNS;
    return *this;
  }
  ...
  {
    cont_RNS->encrypt_float(*c._c, *p._m);
  }

  void decrypt(plaintext_t &p, ciphertext_t &c)
  {
    cont_RNS->decrypt_float_not_ntt(*p._p, *c._c);
  }

  void multiply(ciphertext_t &c1, ciphertext_t &c2)
  {
    cont_RNS->mult_float(*c1._c, *c2._c);
  }

  void relinearise(ciphertext_t &c1)
  {
    cont_RNS->relinearisation_float(*c1._c);
  }
